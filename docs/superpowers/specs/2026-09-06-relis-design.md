...
- **Négatifs (plan 4).** Les épisodes du plan 2a ne contenaient aucun appât : tout fait présent dans l'historique répondait à la question. Le modèle en a appris « s'arrêter sur la première phrase en forme de fait » et « s'arrêter à la première mention du sujet », deux raccourcis indiscernables de la bonne règle sur ces données et démentis par la sonde `relis.eval.distractor` au pas 500. Chaque famille reçoit donc, dans 70 % des épisodes, un à trois **tours-appâts** tirés sur des indices libres de l'historique : un fait en forme de fait sur un autre sujet (« Mon code postal est 75012. » quand la question porte sur le serveur) ou une mention du sujet de la question sans sa valeur (« Parlons de la réunion. »). L'oracle ne change pas : un appât n'est jamais nécessaire, donc il est lu et suivi de CONT. Les indices des appâts sont conservés dans le cas pour que l'évaluation en autonomie compte les « arrêts sur appât ». La classe SUJETS croise DOC_TOPICS sur « la réunion » et « le serveur » : pour une question « qu'ai-je dit sur X », aucun fait de DOC_TOPICS portant sur X n'est tiré comme appât.
- **Historiques longs (plan 4).** L'aiguille (§9) réussit à 0,95 jusqu'à 1 ko d'historique, la longueur maximale vue à l'entraînement (14 paires), et s'effondre au-delà (0,35 à 4 ko, 0 à 16 ko) parce que le modèle s'arrête après quelques segments : il n'a jamais eu à maintenir CONT sur des centaines de segments. Une famille `long_history` (fait dans la moitié ancienne d'un historique de 20 paires jusqu'au maximum que le budget de ruban permet, ≈ 125 octets de ruban par paire) comble l'écart en deux étapes : rubans de 15 ko dans des séquences de 16 384 (≈ 115 paires, couvre 4 ko), puis rubans de 60 ko dans des séquences de 65 536 (≈ 475 paires, couvre 16 ko) — le curriculum sur la longueur annoncé ci-dessous, rendu possible par le traitement par morceaux de 512 positions (la mémoire d'activations par morceau ne dépend pas de la longueur). Le budget de ruban devient un paramètre (`max_tape_bytes`) du générateur et de l'empaquetage. Au-delà de 60 ko, l'aiguille mesure l'**extrapolation** de la récurrence, ce qui est son objet.
- **Échelle en V1 (plan 2a) et suite.** Les rubans d'entraînement tiennent dans 16 384 octets : les épisodes du plan 2a ont au plus 8 documents de quelques ko et 14 tours, et tous les segments d'historique sont lus (SKIP n'est entraîné que sur les documents). Les documents longs, la troncature « côté ancien », des segments d'historique sautés et des questions à 30 tours de distance sont des **objectifs explicites des plans 2b et 3** (curriculum sur la longueur : 8 192 → 16 384 → séquences plus longues une fois les réinitialisations par ruban en place).
...
Entropie croisée sur tout le ruban, pondérée par type de position : octets de réponse et de note ×1 ; codes de contrôle ×5 (rares, critiques) ; octets balayés et octets de la requête ×0,1 (signal LM gratuit) ; en-têtes ×0,1 ; marqueurs insérés par le contrôleur, rembourrage, octets d'adresse et octets développés d'un rappel ×0 (jamais prédits par la tête d'octets). Les séquences de **rappel du corpus de pré-entraînement** (20 % des séquences ruban) pèsent ×0,1 comme du texte balayé : au poids 1 elles représenteraient la moitié du gradient et les décisions un huitième ; à 0,1 le rappel pèse ≈ 10 % et les décisions ≈ 25 %. Le script d'empaquetage imprime ce mélange (part de chaque classe et nombre de passages impliqué par la configuration) : il doit être relu avant tout lancement. La tête de rappel a sa propre entropie croisée sur les cellules, pondérée ×1.
