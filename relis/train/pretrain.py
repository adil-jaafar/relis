...
def resolve_data_path(path: str, search_roots=DEFAULT_SEARCH_ROOTS) -> str:
    ...
    if not found:
        raise FileNotFoundError(
            f"{path} introuvable et aucun '{name}' sous {list(search_roots)} ; "
            f"passer --override data.train_bin=… data.val_bin=…")
    raise FileNotFoundError(
        f"{path} introuvable et plusieurs '{name}' candidats : {found} ; "
